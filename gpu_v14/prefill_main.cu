    ...
    auto try_add_request = [&](int slot, PrefillTask task) -> bool {
      ...
      if (enable_prefix_cache) {
        ...
        if (hit_blocks > 0) {
          ...
          // fprintf(stderr, "[P.prefill] prefix cache hit: req_id=%d hit_blocks=%d
          // hit_tokens=%d\n",
...
void p_send_thread(PNode& pnode, BlockPool* pool, SafeQueue<PrefillResult>& result_queue,
  ...
  while (result_queue.pop(result)) {
    ...
    // fprintf(stderr, "[P.send] 准备发送 d_node_id=%d req_id=%d\n", result.d_node_id, result.req_id);
    pnode.send_response(result.d_node_id, resp);
    // fprintf(stderr, "[P.send] send_response 完成\n");
