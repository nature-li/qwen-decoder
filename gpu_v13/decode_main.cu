...
/**
 * 心跳线程：每隔 HEARTBEAT_INTERVAL_S 秒发一次心跳给 P节点
 * decode 完成后通过 running 标志退出
 */
void d_heartbeat_thread(DNode& dnode, std::atomic<bool>& running) {
  fprintf(stderr, "[D.heartbeat] 启动，间隔 %ds\n", HEARTBEAT_INTERVAL_S);
  while (running) {
    std::this_thread::sleep_for(std::chrono::seconds(HEARTBEAT_INTERVAL_S));
    if (!running) break;
    dnode.send_heartbeat();
    fprintf(stderr, "[D.heartbeat] 发送心跳 d_node_id=%d\n", dnode.d_node_id);
  }
  fprintf(stderr, "[D.heartbeat] 线程退出\n");
}
...
void d_send_thread_func(EncodeQueue& encode_queue, DNode& dnode) {
  ...
  while (encode_queue.pop(&req)) {
    ...
    req_msg.d_node_id = dnode.d_node_id;
...
int main(int argc, char** argv) {
  ...
  std::atomic<bool> heartbeat_running{true};  // 心跳开关
  ...
  // 心跳线程
  std::thread heartbeat_th(d_heartbeat_thread, std::ref(dnode), std::ref(heartbeat_running));
  ...
  // 正常退出，发 SHUTDOWN 给 P节点
  fprintf(stderr, "[main] 发送 SHUTDOWN 给 P节点\n");
  dnode.send_shutdown();
