...
void comm_thread_func(
  ...
  for (int i = 0; i < (int)user_inputs.size(); i++) {
    ...
    info.prompt_tokens.insert(info.prompt_tokens.begin(),
                               decoder->get_tokenizer().bos_token_id);
  ...
  for (auto& info : reqs) {
    int body_size = sizeof(PrefillRequest) + info.n_tokens * sizeof(int);
    ...
    memcpy(body.data() + sizeof(PrefillRequest),
           info.prompt_tokens.data(), info.n_tokens * sizeof(int));
  ...
  // 通知 P节点所有请求已发完
  tcp_send_msg(tcp_fd, MsgType::ALL_SENT, nullptr, 0);
  ...
  for (auto& info : reqs) {
    ...
    double ttft_ms = std::chrono::duration<double, std::milli>(
        t_ttft_end - t_ttft_start).count();
    fprintf(stderr, "[comm] TTFT req_id=%d: %.1fms first_token=%d\n",
            resp.req_id, ttft_ms, resp.first_token);
    ...
    nccl_recv_kv(nccl, pool->get_k_cache(), pool->get_v_cache(),
                  slots, info.n_tokens, n_layers, kv_dim);
    auto t_kv_end = std::chrono::steady_clock::now();
    double kv_ms = std::chrono::duration<double, std::milli>(
        t_kv_end - t_kv_start).count();
    double kv_mb = (double)info.n_tokens * n_layers * kv_dim * 2
                   * sizeof(__half) / 1024 / 1024;
    fprintf(stderr, "[comm] KV cache req_id=%d: %.1fMB in %.1fms (%.1f MB/s)\n",
            info.req_id, kv_mb, kv_ms, kv_mb / kv_ms * 1000);
